...
public class MyDataWareHouse {
	...
	public void loadFileStatus(DataProcess dp, String table_name){
		for (int i = 0; i < this.configNames.size(); i++) {
			...
			if (imp_dir.exists()) {
				...
				for(File f: listFile){
						dp.getControlDb().insertLogFileStatus(import_dir, table_name, f.getName(), config_id, FILE_STATUS_READY,timestamp);
				}
	...
	public boolean checkTransformReady(){
		return false;
		
	}
	
	public void ExtractToDB(DataProcess dp, String configName) {
	...
	public static void main(String[] args) {
		MyDataWareHouse dw=new MyDataWareHouse();
		dw.addConfigName("file_xlsx");
	   ...
	   dw.loadFileStatus(dp,"log_file");
       dw.extractToStagingDB(dp);
