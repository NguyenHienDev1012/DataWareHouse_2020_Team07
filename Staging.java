...
public class Staging {
				...
				for (File f : listFile) {
					if (listFileNameCurrent.size() > 0) {
						if (!listFileNameCurrent.contains(f.getName())) {
							dp.getControlDb().insertLogFileStatus(table_name, f.getName(), config_id, FILE_STATUS_READY,
									timestamp);
						}
					} else {
						dp.getControlDb().insertLogFileStatus(table_name, f.getName(), config_id, FILE_STATUS_READY,
	...
	public boolean extractToStagingDB(int id_config) throws SQLException {
		...
		for (File file : listFile) {
			...
			if (log_file.getData_file_config_id() == config_id && file_status.equals("ER")) {
				...
				if (values != null) {
					...
					String timestamp = getCurrentTime();
					controlDb.truncateTable("stagingdb", target_table);
...
//							System.out.println(timestamp);
							dp.getControlDb().updateLogAfterLoadingIntoStagingDB(table, file_status, timestamp,
									stagin_load_count, file.getName());
						}
						try {
							int id_file = dp.getControlDb().selectIDFile("data_file_id", "log_file", file.getName());
							System.out.println(" start load to dw");
							dw.loadToDW(id_file);
							System.out.println(" end load to dw");
