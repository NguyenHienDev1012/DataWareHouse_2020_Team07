...
public class ControlDB {
	...
	private String sql="";
	/*
	 * This is constructor for class Controldb
	 */
	...
	public int selectDataFileId(String field,String table, int data_file_config_id) {
		sql = "SELECT " + field + " FROM " +table + " WHERE data_file_config_id=?";
		System.out.println(sql);
		try {
			ptmt = DBConnection.createConnection(this.source_db).prepareStatement(sql);
			ptmt.setInt(1,data_file_config_id);
	...
	public boolean updateLogAfterLoadingIntoStagingDB(String table, String file_status, String config_id, String timestamp,
			String stagin_load_count, int data_file_id) {
		  sql = "update "+table+ " set file_status=?, staging_load_count=? where data_file_id='" + data_file_id + "'";
		  System.out.println(sql);
		try {
			ptmt = DBConnection.createConnection(this.source_db).prepareStatement(sql);
